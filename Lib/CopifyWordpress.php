 ...
 * Plugin dir name
 */
	public $copifyDirName = COPIFY_PLUGIN_DIR;
 ...
 **/
	public function CopifyOrder() {
		try {
			...
			// Get category, budget and status resources from API
			$CopifyCategories = $this->CopifyGetJobCategories();
			$CopifyBudgets = $this->CopifyGetJobBudgets('Professional');
			$CopifyStatuses = $this->CopifyGetJobStatuses();
			// Create a plain array of categories with the ID as the key
			$categoryList = $this->CopifyFlatten($CopifyCategories);
			$budgetList = $this->CopifyFlatten($CopifyBudgets);
 ...
 **/
	public function CopifyGetJobCategories() {
		$CopifyCategories = get_transient('CopifyCats');
		if ($CopifyCategories !== false) {
			return $CopifyCategories;
		}
		$CopifyCategories = $this->Api->jobCategories();
		$this->wordpress('set_transient', 'CopifyCats', $CopifyCategories['job_categories'], 86400);
 ...
 **/
	public function CopifyGetJobBudgets() {
		$CopifyBudgets = $this->wordpress('get_transient', 'CopifyBudget');
		if ($CopifyBudgets !== false) {
			return $CopifyBudgets;
		}
		$CopifyBudgets = $this->Api->jobBudgets();
		$this->wordpress('set_transient', 'CopifyBudget', $CopifyBudgets['job_budgets'], 86400);
 ...
 **/
	public function CopifyGetJobStatuses() {
		$CopifyStatuses = $this->wordpress('get_transient', 'CopifyStatus');
		if ($CopifyStatuses !== false) {
			return $CopifyStatuses;
		}
		$CopifyStatuses = $this->Api->jobStatuses();
		$this->wordpress('set_transient', 'CopifyStatus', $CopifyStatuses['job_statuses'], 86400);
 ...
 **/
	public function CopifyGetUserProfile($id = 1) {
		$CopifyGetUserProfile = $this->wordpress('get_transient', 'CopifyGetUserProf'.$id);
		if ($CopifyGetUserProfile !== false) {
			return $CopifyGetUserProfile;
		}
		$CopifyGetUserProfile = $this->Api->usersView($id);
		$this->wordpress('set_transient', 'CopifyGetUserProf' . $id, $CopifyGetUserProfile, 604800);
		return $CopifyGetUserProfile;
	}
	...
	public function CopifyRequestFilter() {
		try {
			...
			if ($expectedToken != $token) {
				throw new Exception('Permission denied: API Key is not set correctly.', 403);
			}
			// Version check only
 ...
 * @author Rob Mcvey
 **/
	public function parseImageAttributionMeta() {
		if (empty($_GET)) {
			return;
		}
 ...
 * @author Rob Mcvey
 **/
	public function autoPublishOrder() {
		// Order ID
		if (!isset($_GET["id"])) {
 ...
 * @author Rob Mcvey
 **/
	public function CopifyCheckThumbnailExtension($ext) {
 ...
 * @author Rob Mcvey
 **/
	public function setUpdateAttachmentMeta($attach_id, $filepath, $meta = array()) {
 ...
 * @author Rob Mcvey
 **/
	public function CopifyCheckImageHost($url) {
		$parts = parse_url($url);
		if (!preg_match("/flickr|copify|pixabay/", $parts['host'])) {
 ...
 * @author Rob Mcvey
 **/
	public function _wp_get_attachment_metadata() {
 ...
 * @author Rob Mcvey
 **/
	public function wordpress($method, $mixed = null) {
		$args = func_get_args();
		if (!isset($args[0])) {
			return false;
		}
