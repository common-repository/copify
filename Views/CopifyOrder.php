			...
			<tr>
				<td>
						...
						<?php
							...
							// Only 1 budget?
							$showBudgetOption = true;
							if (count($budgetList) == 1) {
								$showBudgetOption = false;
							} else {
								echo '<label for="job_budget_id">Choose writer standard</label>';
							}
							?>
							...
							foreach($budgetList as $budget_id => $budget_name) {
								...
								echo sprintf('<input name="job_budget_id" type="radio" value="%d" %s style="%s" />' , $budget_id, $checked, $radStyle);
								if ($showBudgetOption) {
									echo sprintf('<span class="CopifyRadioSpan budget %s">%s</span>' , $lowerCaseName , $budget_name);
								}
							}
						?>
					</div>
